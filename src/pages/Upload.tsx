...
const Upload = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
        toast({ title: 'You must be logged in to create a profile.', variant: 'destructive' });
        setLoading(false);
        return;
    }
    ...
    const { error: imagesError } = await supabase.from('profile_images').insert(imageInserts);
